...
再試験のReady：ユーザーの「リトライしてみましよう」により、同じKnowledge版・設問・採点基準での再試験を開始する。説明・診断だけが対象で、DoneはHuman Review待ち。

R3の再開：送信先と資料を明示した確認へのユーザー回答「codexにしてもらうか」を受け、同じ資料をOpenAI Codexで処理する再試行を承認済みとして開始する。旧Session・旧回答は引き継がない。
