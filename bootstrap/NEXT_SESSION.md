...
状態：修正はPR #28で承認・main反映済み。B-12の環境成立と改訂後の受入試験が未完了。これは担当者向けの索引であり、受験者へ渡す入力ではない。
...
- **改訂は完了**：[PR #28](https://github.com/tanakakeisuke-github/tOS/pull/28)はmainへ反映済み、Issue #27は完了。次はIssue #25の環境成立確認から進む。
- **試験環境の成立確認は未達**：[環境確認記録](../outcomes/B12_ENVIRONMENT_CHECK.md)の条件・証拠から続ける。追加のapp-server検証ではモデル応答・専用Readerの許可/拒否まで確認済み。残るSkill取得・委譲経路の制限と監査を解決する。同じReader成功確認だけを繰り返さず、未検証の全経路隔離を達成済みと扱わない。
